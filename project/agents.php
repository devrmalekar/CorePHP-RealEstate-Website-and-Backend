...
function sqlQueryValue()
	...
	function homerentsale($value){
		...
		$leftRecord = $totalRecord - ($page * $recordLimit);
	 	...
	 	$sql = "SELECT CONCAT(FirstName, ' ', LastName) AS Name ,Description, Contact,Email, imgURL FROM `agentdetail`";
  ...
  <div class="col-lg-8  col-lg-offset-2 col-sm-12">
      ...
      <?php 
            ...
            $resultHP = homerentsale($value);
               ...
               {
				   $imgUrl = substr($row["imgURL"],2, (strlen($row["imgURL"]) - 1)); 
				   echo '  <div class="row">
        <div class="col-lg-2 col-sm-2 "><img src="'.$imgUrl.'" class="img-responsive"  alt="agent name" style="height: 100px;"></div>
        <div class="col-lg-7 col-sm-7 "><h4>'.$row["Name"].'</h4><p>'.$row["Description"].'</p></div>
        <div class="col-lg-3 col-sm-3 "><span class="glyphicon glyphicon-envelope"></span> <a href="mailto:'.$row["Email"].'">'.$row["Email"].'</a><br>
        <span class="glyphicon glyphicon-earphone"></span>'.$row["Contact"].'</div>
      </div>';
			   }
	 		if ($totalRecord > 11){
