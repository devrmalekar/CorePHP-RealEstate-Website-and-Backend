<?php 
	...
	function sqlQueryValue()
	{
		...
		if(isset($_GET['purpose']) && !empty($_GET['purpose'])){
			$value=" WHERE `Purpose`='".mysql_real_escape_string($_GET['purpose'])."' ";
		}
		
		if(isset($_GET['pricefrom']) && !empty($_GET['pricefrom'])){
			if(strpos($value, "WHERE")){
				$value = " AND `Price`>'".mysql_real_escape_string($_GET['pricefrom'])."'";
			} else {
		...
		if(isset($_GET['priceto']) && !empty($_GET['priceto'])){
			if(strpos($value, "WHERE")){
				$value = " AND `Price`<'".mysql_real_escape_string($_GET['priceto'])."'";
			} else {
            ...
            <div class="sortby clearfix">
                ...
                <div class="pull-right" style="float:left;">
                  <select class="form-control" id="purpose" >
                      <option>View</option>
                      <option value="LandRent" <?php if ($_GET['purpose'] == 'LandRent') { ?> selected="selected" <?php } ?>>Land for Rent</option>
                      <option value="LandSale" <?php if ($_GET['purpose'] == 'LandSale') { ?> selected="selected" <?php } ?>>Land for Sale</option>
                  </select>
                </div>
         ...
         <!-- properties -->
            ...
            $resultHP = homerentsale($value);
               while ($row = mysql_fetch_array($resultHP))
                {
                    $href = "property-detail.php?LandName=".str_replace(" ","%20",$row['LandName']);
                ...
                <span data-toggle="tooltip" data-placement="bottom" data-original-title="Area in sq ft">'. $row["Area"].'</span>
